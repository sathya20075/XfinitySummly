package org.apps.summly.httpUtilities;
			 ...
			 for(final HttpGet request :requests){
				 httpClient.execute(request, new FutureCallback<HttpResponse>(){
					...
					public void completed(HttpResponse resp) {
						...
						String newsKey=null;						
						HashMap<String,String> newsDict;
						latch.countDown();						
						...
						try {
							...
							newsDict = new HashMap<String,String>();
							...
							{								
								newsKey = "Politics-"+Utilities.getCurrentDateAndTime();
								newsDict.put(newsKey, URLResponse);
							}
							else if(request.getURI().toString().indexOf("Business") != -1)
							{
								newsKey = "Business-"+Utilities.getCurrentDateAndTime();
								newsDict.put(newsKey, URLResponse);
							}
							else if(request.getURI().toString().indexOf("Sports") != -1)
							{
								
								newsKey = "Sports-"+Utilities.getCurrentDateAndTime();
								newsDict.put(newsKey, URLResponse);
							}
							else
							{
							...
							dbCollection.insert(new BasicDBObject(newsDict));
